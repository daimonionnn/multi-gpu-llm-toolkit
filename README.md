...
- [ ] Windows: AMD dGPUs do not enumerate over OCuLink here. `Non-Eval Discrete GPU Support` was the leading suspect until **Framework Desktop’s BIOS was extracted and ships it disabled at the same offset while AMD cards enumerate** — so the cause is still open. What stands: **no channel can write `AMD_PBS_SETUP`**, and `PCIE x4 DT Slot Power Enable` is the next byte to read ([doc/oculink-amd-dgpu.md](doc/oculink-amd-dgpu.md))
